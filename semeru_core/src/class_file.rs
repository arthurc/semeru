...
named!(cp_info<()>, do_parse!(tag: be_u8 >> (())));

named!(
    class_file<()>,
    do_parse!(
        cafebabe >> minor_version: be_u16 >> major_version: be_u16 >> constant_pool_count: be_u16
...
mod tests {
    ...
    mod class_file {
        ...
        fn given_a_class_file_then_it_should_be_parsable() {
            assert!(class_file(&[0xCA, 0xFE, 0xBA, 0xBE, 0x10, 0x55]).is_done());
        }
    }
